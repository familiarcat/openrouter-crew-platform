...
collect_from_file() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    for entry in "${VAR_MAP[@]}"; do
      ...
      # Check only Canonical, Alt1, and Alt2 indices (skip description)
      for i in 0 1 2; do
...
ensure_generated_secrets() {
  ...
  if [[ -z "${RESOLVED[N8N_WEBHOOK_SECRET]:-}" ]]; then
    local new_secret
    ...
    RESOLVED["N8N_WEBHOOK_SECRET"]="$new_secret"
    SOURCES["N8N_WEBHOOK_SECRET"]="auto-generated"
    MAPPING["N8N_WEBHOOK_SECRET"]="N8N_WEBHOOK_SECRET"
  fi
}
