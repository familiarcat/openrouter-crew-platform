...
export class MaintenanceService {
  ...
  private scaffoldPackage(dir: string, pkgJsonContent: object, tsConfigContent: object, srcContent: string) {
    ...
    fs.mkdirSync(path.join(fullDir, 'src'), { recursive: true });
    ...
    fs.writeFileSync(path.join(fullDir, 'src/index.ts'), srcContent);
...
`;
  ...
  public async runMaintenance() {
    ...
    this.run('pnpm', ['build', '--filter', '@openrouter-crew/agent-orchestration']);
    this.run('pnpm', ['build', '--filter', '@openrouter-crew/crew-api-client']);
