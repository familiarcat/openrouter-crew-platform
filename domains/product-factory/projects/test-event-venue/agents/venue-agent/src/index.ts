...
app.post('/tools/:toolName', async (req, res) => {
  ...
  console.log(`[venue-agent] Executing tool: ${toolName}`);
  
  try {
    // Log to database
    await pool.query(
      'INSERT INTO agent_logs (agent_name, action, payload) VALUES ($1, $2, $3)',
      ['venue-agent', toolName, JSON.stringify(args)]
    );
    ...
    if (toolName === 'consult_crew') {
      ...
      console.log(`[venue-agent] Consulting Crew Member: ${member}`);
    ...
    console.error(`[venue-agent] Error executing tool:`, error);
    res.status(500).json({ 
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    });
...
// Start server
app.listen(PORT, () => {
  console.log(`[venue-agent] MCP Server running on port ${PORT}`);
});

// Graceful shutdown
process.on('SIGTERM', async () => {
  console.log(`[venue-agent] SIGTERM received, closing server...`);
  await pool.end();
  process.exit(0);
});
