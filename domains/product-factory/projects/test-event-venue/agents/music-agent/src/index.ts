...
app.post('/tools/:toolName', async (req, res) => {
  ...
  console.log(`[music-agent] Executing tool: ${toolName}`);
  
  try {
    // Log to database
    await pool.query(
      'INSERT INTO agent_logs (agent_name, action, payload) VALUES ($1, $2, $3)',
      ['music-agent', toolName, JSON.stringify(args)]
    );
    ...
    if (toolName === 'consult_crew') {
      ...
      console.log(`[music-agent] Consulting Crew Member: ${member}`);
    ...
    console.error(`[music-agent] Error executing tool:`, error);
    res.status(500).json({ 
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    });
...
// Start server
app.listen(PORT, () => {
  console.log(`[music-agent] MCP Server running on port ${PORT}`);
});

// Graceful shutdown
process.on('SIGTERM', async () => {
  console.log(`[music-agent] SIGTERM received, closing server...`);
  await pool.end();
  process.exit(0);
});
