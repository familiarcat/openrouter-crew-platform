...
import { ModelChoice } from '@openrouter-crew/shared-cost-tracking';
...
Please provide a corrected version of the task: ${originalTask}`;
  ...
  getRecommendedModel(
    ...
    // at the current tier, upgrade to the next level of intelligence.
    if (result.score < 0.4 || retryCount >= 1) {
      const tiers = [ModelChoice.HAIKU, ModelChoice.SONNET, ModelChoice.OPUS];
