 ...
 */
export const DEFAULT_DECAY_POLICIES = {
    eternal: {
...
export class MemoryDecayService {
     ...
     */
    calculateConfidenceDecay(initialConfidence, daysPassed, dailyDecayRate) {
        // Exponential decay
     ...
     */
    calculateExpirationDate(createdAt, tier) {
     ...
     */
    calculateRecoveryDeadline(deletedAt, tier) {
        const policy = this.getDecayPolicy(tier);
     ...
     */
    calculateCurrentConfidence(memory) {
        ...
        return this.calculateConfidenceDecay(memory.confidence_level, daysPassed, policy.dailyDecayRate);
    }
    /**
     * Get decay metrics for a memory
     */
    getDecayMetrics(memory) {
        const policy = this.getDecayPolicy(memory.retention_tier);
        const created = new Date(memory.created_at);
        const now = new Date();
        const daysPassed = (now.getTime() - created.getTime()) / (1000 * 60 * 60 * 24);
        ...
        if (currentConfidence <= policy.minConfidence) {
            ...
            const daysToThreshold = Math.log(policy.minConfidence / currentConfidence) / Math.log(1 - policy.dailyDecayRate);
            daysUntilExpiration = Math.max(0, daysToThreshold);
        }
        const isExpired = currentConfidence <= policy.minConfidence ||
     ...
     */
    getExpirationReason(memory, currentConfidence, daysPassed, policy) {
        if (currentConfidence <= policy.minConfidence) {
        ...
        if (memory.deleted_at) {
            const daysSinceDeleted = (new Date().getTime() - new Date(memory.deleted_at).getTime()) / (1000 * 60 * 60 * 24);
     ...
     */
    async findExpiringMemories(crewId, daysUntilExpiration = 7) {
        ...
        for (const memory of memories) {
            ...
            if (metrics.daysUntilExpiration > 0 &&
     ...
     */
    async findMemoriesReadyForHardDelete(crewId) {
        const { data, error } = await this.supabase
            .from('crew_memory_vectors')
            .select('*')
            .eq('crew_id', crewId)
     ...
     */
    async findLowConfidenceMemories(crewId, confidenceThreshold = 0.3) {
        const { data, error } = await this.supabase
            .from('crew_memory_vectors')
            .select('*')
            .eq('crew_id', crewId)
     ...
     */
    async softDeleteExpiredMemories(crewId, context) {
        ...
        const memories = (expiredMemories || []);
        let deletedCount = 0;
        for (const memory of memories) {
     ...
     */
    async hardDeleteExpiredMemories(crewId) {
     ...
     */
    async getRetentionStatistics(crewId) {
