/**
 * Content Store API - DDD-Compliant
 * 
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (MCP_API_KEY) {
      try {
        const mcpResponse = await fetch(`${MCP_BASE_URL}/api/content/store`, {
          method: 'POST',
          headers: {
    ...
    // FALLBACK: Try n8n
    try {
      const n8nResponse = await fetch(`${N8N_URL}/webhook/project-content-store`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Source': 'alex-ai-dashboard-api'
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to store content' },
      { status: 500 }
    );
  } catch (error: any) {
    return NextResponse.json(
