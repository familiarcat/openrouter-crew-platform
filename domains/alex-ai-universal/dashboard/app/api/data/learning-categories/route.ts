/**
 * Learning Categories API - DDD-Compliant
 * 
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (MCP_API_KEY) {
      try {
        const mcpResponse = await fetch(`${MCP_BASE_URL}/api/data/learning-categories`, {
          method: 'GET',
          headers: {
    ...
    // FALLBACK 1: Try n8n
    try {
      const n8nResponse = await fetch(`${N8N_URL}/webhook/learning-categories`, {
        method: 'GET',
