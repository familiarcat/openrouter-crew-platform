/**
 * Content Retrieve API - DDD-Compliant
 * 
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (MCP_API_KEY) {
      try {
        const mcpResponse = await fetch(`${MCP_BASE_URL}/api/content/retrieve?projectId=${projectId}`, {
          method: 'GET',
          headers: {
    ...
    // FALLBACK: Try n8n
    try {
      const n8nResponse = await fetch(`${N8N_URL}/webhook/project-content-retrieve?projectId=${projectId}`, {
        method: 'GET',
        headers: {
          'X-Source': 'alex-ai-dashboard-api',
          'Cache-Control': 'no-cache'
