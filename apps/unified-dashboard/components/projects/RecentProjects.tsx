...
import { SprintBadge } from "@/components/sprints/SprintIndicator";
...
interface Project {
  ...
  progress: number;
  updatedAt: string;
  domain?: Domain;  // Singular - a project belongs to ONE domain
  scores?: {
    demand: number;
    effort: number;
    monetization: number;
...
function DomainBadge({ domain }: { domain: Domain }) {
  return (
    ...
    >
      ...
      />
      <span style={{ opacity: 0.9 }}>{domain.name}</span>
    </div>
  );
}
...
function ProjectPreview({
  ...
  const domainColor = project.domain?.color || theme.accent;
    ...
    >
      <div
        ...
        style={{
          ...
          background: `linear-gradient(180deg, rgba(13,16,34,.95), rgba(11,15,29,.85)), radial-gradient(ellipse 300px 200px at 0% 0%, ${domainColor}20 0%, transparent 60%)`,
          height: "100%",
        }}
        onMouseEnter={(e) => {
          e.currentTarget.style.transform = "translateY(-2px)";
          e.currentTarget.style.boxShadow = `0 8px 24px ${domainColor}20`;
        }}
        onMouseLeave={(e) => {
      ...
      >
        ...
        >
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {/* Domain Badge - A project belongs to ONE domain */}
            {project.domain && (
          ...
          >
            <div
              style={{
                ...
                height: "100%",
                background: `linear-gradient(90deg, ${statusColor}, ${domainColor})`,
                borderRadius: 2,
...
export function ProjectStats({ theme }: { theme: { accent: string } }) {
  ...
  async function loadStats() {
    try {
      ...
      // Count unique domains (a project belongs to ONE domain)
      const uniqueDomainIds = new Set(
        projects
          .filter((p: Project) => p.domain?.id)
          .map((p: Project) => p.domain!.id)
      );
      const domainsCount = uniqueDomainIds.size;

      setStats({ total: projects.length, active, domains: domainsCount });
    } catch (error) {
  ...
  return (
    <div style={{ display: "flex", gap: 12 }}>
      ...
      <div
        ...
        title="Number of unique domains with projects"
      >
        <div style={{ fontSize: 24, fontWeight: "bold", color: "#3b82f6" }}>
          {stats.domains}
        </div>
        <div style={{ fontSize: 10, color: "var(--muted)" }}>
          {stats.domains === 1 ? 'Domain' : 'Domains'} Active
        </div>
      </div>
    </div>
  );
